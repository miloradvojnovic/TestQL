...
def request_obj_processor(request):
    tag_values = set()
...
def response_obj_processor(response):
    tag_values = set()
...
def examples_obj_processor(examples): 
    values = set()
...
def check_some_semantics(model, metamodel):
    tag_values = model.request
    example_values = set()
    for case in model.cases:
        tag_values = tag_values.union(case.response)
        example_values = example_values.union(case.examples)
